...
#cette fonction modifie les valeurs de x et y en leurs attribuant les coordonées de la grille corresponand au carré sur lequel on a cliqué
def click_inter_grille(grille):                     #prend une grille en parametre (tableau n*n)
    if pygame.mouse.get_pressed()[0]:               #renvoie 1 si le le click gauche est activé
        ...
        if (screen.get_at(click)==BLANC):           #ne fait quelque chose que si l'on a cliqué sur du blanc
...
BLANC=(255,255,255) #couleur que l'on utilise pour les rectangles
while not done:
    ...
    for x in range(4):
        for y in range(4):
            pygame.draw.rect(screen,BLANC,pygame.Rect(x*(cot+mar),y*(cot+mar),cot,cot)) #dessine la "grille de dimension n" 
                                                                                        #sous formes de n*n rectangles avec une marge "mar"
                                                                                
    if(click_inter_grille(grille)):
        print(click_inter_grille(grille))        
